...
class PdfAnalyst():
    ...
    def analysis_with_keywords_system1(self, name, keywords_1, keywords_2, keywords_3):
        ...
        if name == "高管致辞":
            ...
            # 段落中含有碳、环保、绿色的句子
            pno_sentences = get_sentences_with_keywords(pno_paragraphs, self.keywords_normal, keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
            # 获取表格和图片数量
            ...
            pno_sentences.extend(get_sentences_with_keywords(self.relevant_pno_paragraphs, keywords_2, keywords_3, ignore_words=self.ignore_words, keywords_type="double"))
    ...
    def analysis_with_keywords_system2(self, name, method, keywords_1, keywords_2, keywords_3):
        ...
        if name == "高管有关双碳目标或碳减排的声明与承诺":
            ...
            pno_paragraphs = get_management_speech_paragraphs(self.pdf.document_info, self.pno_start, self.pno_end)
            # 段落中含有碳、气候、节能、能源的句子
            pno_sentences = get_sentences_with_keywords(pno_paragraphs, ["碳", "气候", "节能", "能源"], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
            # sentences = self.get_nonrepeated_sentences(pno_sentences) # 去除与之前指标相重复的句子
            ...
            pno_sentences = get_sentences_with_keywords(pno_paragraphs, ["管理机制", "制度", "流程", "整体", "气候变化", "能源"], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
            # sentences = self.get_nonrepeated_sentences(pno_sentences) # 去除与之前指标相重复的句子
            sentences = remove_duplicate([item[1].strip() for item in pno_sentences])
            content = "\n".join(sentences) # 拼接成字符串
            score = 1 if len(sentences) else 0 # 有句子则得分1分
            ...
            pno_sentences = get_sentences_with_keywords(pno_paragraphs, ["碳", "气候变化", "节能", "能源"], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
            # sentences = self.get_nonrepeated_sentences(pno_sentences) # 去除与之前指标相重复的句子
            sentences = remove_duplicate([item[1].strip() for item in pno_sentences])
            content = "\n".join(sentences) # 拼接成字符串
            score = 1 if len(sentences) else 0 # 有句子则得分1分
            ...
            # 段落中含有去年和前年的句子
            pno_sentences = get_sentences_with_keywords(pno_paragraphs, [last_year, last_last_year], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
            # sentences = self.get_nonrepeated_sentences(pno_sentences) # 去除与之前指标相重复的句子
            sentences = remove_duplicate([item[1].strip() for item in pno_sentences])
            content = "\n".join(sentences) # 拼接成字符串
            score = 1 if len(sentences) else 0 # 有句子则得分1分
            ...
            if method == "关键词":
                if name == "确定碳排放核算责任的运营边界（依据范围一、范围二、范围三界定）":
                    ''' 涵盖了几个范围得几分 '''
                    range_1 = get_sentences_with_keywords(self.relevant_pno_paragraphs, ["范围一", "范畴一"], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
                    range_2 = get_sentences_with_keywords(self.relevant_pno_paragraphs, ["范围二", "范畴二"], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
                    range_3 = get_sentences_with_keywords(self.relevant_pno_paragraphs, ["范围三", "范畴三"], keywords_2=[], ignore_words=self.ignore_words, keywords_type="single")
